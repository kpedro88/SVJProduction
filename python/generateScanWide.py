...
parser.add_argument("-w","--width", dest="width", type=float, default=0.0, help="Zprime width")
...
params = OrderedDict([
    ...
    ("rinv", [0., 0.05] + [float(x)/10 for x in range(1,11,1)]),
    ("alpha", ["peak", "high", "low"]),
])
...
for point in sorted(sigs):
    ...
    helper.setModel("s",mZprime,mDark,rinv,alpha,width=args.width)
...
with open("SVJ_Scan_wZprime-"+str(args.width)+"_"+str(args.year)+"_"+tune_suff+"_cff.py",'w') as ofile:
